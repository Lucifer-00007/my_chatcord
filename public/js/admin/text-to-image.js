...
async function loadImageApiList() {
  if (!window.adminUtils) {
    console.error(
      '[text-to-image.js] adminUtils is undefined, but user is authenticated. Possible server/network error.'
    );
  ...
  try {
    imageApiList.innerHTML =
      '<div class="loading"><i class="fas fa-spinner fa-spin"></i> Loading Image APIs...</div>';

    // First try to get active APIs, fallback to all APIs if there's an error
    let apis = [];
    try {
      apis = await window.adminUtils.makeApiRequest(
        '/api/admin/image-apis/active'
      );
    } catch (activeErr) {
      console.warn(
        'Failed to fetch active APIs, falling back to all APIs:',
        activeErr
      );
...
async function handleImageFormSubmit(e) {
  ...
  try {
    ...
    showNotification(
      `Image API ${isEditing ? 'updated' : 'saved'} successfully`,
      'success'
    );
    resetImageForm();
...
async function loadGlobalSettings() {
  ...
  try {
    ...
    const sizesRes = await window.adminUtils.makeApiRequest(
      '/api/admin/image-apis/settings/sizes'
    );
    ...
    const stylesRes = await window.adminUtils.makeApiRequest(
      '/api/admin/image-apis/settings/styles'
    );
    ...
    if (stylesRes.values?.length > 0) {
      stylesRes.values.forEach((style) => {
        const styleId =
          style.id || style.name.toLowerCase().replace(/\s+/g, '-');
        const div = document.createElement('div');
...
function addGlobalOption(type) {
  if (type === 'size') {
    ...
    if (!size || !/^\d+x\d+$/.test(size)) {
      showNotification(
        'Invalid size format. Please use format: widthxheight',
        'error'
      );
      return;
    }
...
async function editImageApi(id, name) {
  try {
    ...
    document.getElementById('image-response-path').value =
...
async function toggleImageApi(id, isActive) {
  try {
    await window.adminUtils.makeApiRequest(
      `/api/admin/image-apis/${id}/toggle`,
      {
        method: 'PATCH',
        body: { isActive },
      }
...
function initToggleHandlers() {
  ...
  apiList.addEventListener('change', async (e) => {
    if (e.target.matches('.api-toggle')) {
      ...
      try {
        const response = await window.adminUtils.makeApiRequest(
          `/api/admin/image-apis/${apiId}/toggle`,
          {
            method: 'PATCH',
            body: { isActive },
          }
