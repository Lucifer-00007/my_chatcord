...
async function loadUsers() {
  ...
  try {
    let users = [];
    if (search) {
      users = await window.adminUtils.makeApiRequest(
...
async function handleUserFormSubmit(event) {
  ...
  try {
    ...
    let method = 'POST';
    const body = { username, email, isAdmin };
    if (mode === 'add') {
...
async function editUser(userId) {
  try {
    const user = await window.adminUtils.makeApiRequest(
...
async function deleteUser(userId) {
  ...
  try {
    await window.adminUtils.makeApiRequest(`/api/admin/users/${userId}`, {
      method: 'DELETE',
    });
