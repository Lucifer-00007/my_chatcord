    ...
    const checkInterval = setInterval(() => {
      ...
      if (window.adminUtils) {
        ...
        window.showNotification(
          'Error: Failed to initialize admin utilities',
          'error'
        );
...
async function initRoomManagement() {
  ...
  elements.roomsContainer.addEventListener('click', async (e) => {
    ...
    document
      .querySelectorAll('#selected-room-name')
      .forEach((el) => (el.textContent = roomName));
  ...
  elements.roomForm.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      showNotification(
        `Room ${method === 'PUT' ? 'updated' : 'created'} successfully`,
        'success'
      );
      hideRoomModal();
      await loadRooms();
  ...
  elements.blockForm.addEventListener('submit', async (e) => {
    ...
    try {
      const blocks = await window.adminUtils.makeApiRequest(
        `/api/admin/room-management/rooms/${roomId}/blocks`
      );
      const userBlock = blocks.find(
        (block) => block.user && block.user._id === userId && block.isActive
      );
      if (userBlock) {
        const username =
          userBlock.user && userBlock.user.username
            ? userBlock.user.username
            : 'User';
        const roomName = elements.selectedRoomName
          ? elements.selectedRoomName.textContent
          : 'this';
        showNotification(
          `${username} is already blocked in the ${roomName} room!`,
          'warning'
        );
        return;
      }
      ...
      showNotification(
        'Could not verify existing blocks. Please try again.',
        'error'
      );
      return;
    }
    ...
    try {
      await window.adminUtils.makeApiRequest(
        '/api/admin/room-management/blocks',
        {
          method: 'POST',
          body: formData,
        }
...
async function loadRooms() {
  ...
  try {
    const rooms = await window.adminUtils.makeApiRequest(
      '/api/admin/room-management/rooms'
    );
    ...
    container.innerHTML =
      blocks
        .map((block) => {
          ...
          const username =
            block && block.user && block.user.username
              ? block.user.username
              : 'Unknown User';
          const reason =
            block && block.reason ? block.reason : 'No reason provided';
          // Show 'Blocked for lifetime' if duration is 9999999
          let durationText;
          if (block && block.duration === 9999999) {
            durationText =
              '<span class="block-lifetime">Blocked for lifetime</span>';
          } else {
            const endDate =
              block && block.endDate ? new Date(block.endDate) : null;
            const time = endDate
          ...
          // Blocked date
          const startDate =
            block && block.startDate ? new Date(block.startDate) : null;
          const blockedDate = startDate
            ? `${startDate.getDate()}/${startDate.getMonth() + 1}/${startDate.getFullYear()}`
            : '--';
          // Blocked by (show username if available)
          const blockedBy =
            block && block.blockedBy && block.blockedBy.username
              ? block.blockedBy.username
              : block && block.blockedByName
                ? block.blockedByName
                : 'Unknown';

          const durationsConstant =
            window.adminUtils.constants?.ROOM_MANAGEMENT?.BLOCK_DURATIONS || [];
          const blockedDuration =
            durationsConstant.find(
              (duration) => duration.value === block?.duration
            )?.label || 'Unknown';
          return `
...
async function showBlockModal() {
  ...
  try {
    ...
    reasonSelect.innerHTML =
      window.adminUtils.constants.ROOM_MANAGEMENT.BLOCK_REASONS.map(
    ...
    window.showNotification(
      `Failed to initialize block user modal: ${err.message}`,
      'error'
    );
  }
}
...
async function loadUsers() {
  ...
  if (!userSelect) {
    window.showNotification(
      'Error: Could not find user selection element',
      'error'
    );
    return;
  }
    ...
    .addEventListener('click', async (e) => {
      ...
      if (target.classList.contains('edit-room')) {
        const roomData = {
          ...
          description:
            roomItem.querySelector('.room-description')?.textContent || '',
        ...
        if (confirm('Are you sure you want to delete this room?')) {
          try {
            await window.adminUtils.makeApiRequest(
              `/api/admin/room-management/rooms/${roomId}`,
              {
                method: 'DELETE',
              }
            );
...
async function removeBlock(blockId) {
  ...
  try {
    await window.adminUtils.makeApiRequest(
      `/api/admin/room-management/blocks/${blockId}`,
      {
        method: 'DELETE',
      }
    );
...
async function loadChatHistory(roomId) {
  ...
  if (!roomId) {
    chatList.innerHTML =
      '<div class="empty-state">Select a room to view chat history.</div>';
    return;
  }
  try {
    const messages = await window.adminUtils.makeApiRequest(
      `/api/admin/room-management/rooms/${roomId}/chats`
    );
    allChatMessages = messages;
    // Populate user filter
    const uniqueUsers = Array.from(
      new Set(messages.map((m) => m.username || 'Unknown'))
    );
    allChatUsers = uniqueUsers;
    if (userFilter) {
      userFilter.innerHTML = `<option value="">All Users</option>${uniqueUsers
...
window.deleteChatMessage = async function (roomId, idx) {
  ...
  try {
    await window.adminUtils.makeApiRequest(
      `/api/admin/room-management/rooms/${roomId}/chats/${idx}`,
      { method: 'DELETE' }
    );
...
function initChatHistoryManagement() {
  ...
  if (roomSelect) {
    window.adminUtils
      .makeApiRequest('/api/admin/room-management/rooms')
      .then((rooms) => {
    ...
    const selectedRoomId =
      document.querySelector('.room-item.active')?.dataset.id;
    if (selectedRoomId) {
