document.addEventListener('DOMContentLoaded', () => {
    ...
    try {
      ...
      if (res.status === 403) {
        modelSelect.innerHTML =
          '<option value="">No AI models available (access denied)</option>';
  ...
  document
    ...
    .insertBefore(newChatBtn, document.querySelector('.chat-sidebar h3'));
  ...
  function addMessageToChat(sender, text, model = null, createdAt) {
    ...
    if (window.hljs) {
      ...
      console.warn(
        '[addMessageToChat] highlight.js (hljs) not found on window!'
      );
    }
